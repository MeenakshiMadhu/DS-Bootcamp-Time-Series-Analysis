...
options = st.sidebar.radio("Go to:", 
    ["1. Dataset & Cleaning", 
     ...
     "8. Real-World ML Models",
     "9. Real-World Forecasting (ARIMA/VAR/LSTM)"])
...
if df_cleaned is not None:
    ...
    # --- PAGE 4: FORECASTING MODELS ---
        ...
        model_type = st.selectbox("Select Model Type", ["Baseline: ARIMA (Univariate)", "Multivariate: VAR", "Deep Learning: LSTM", "Comparison: ARIMA vs LSTM", "Comparison: All Models"])
        ...
        # --- MODEL 4: Comparison ARIMA vs LSTM ---
            ...
            if st.button("Run Comparison"):
                with st.spinner("Training both models..."):
                    ...
                    ax.set_title("Model Comparison: ARIMA vs. Deep Learning (LSTM)")
                    ax.legend()
                    st.pyplot(fig)
        ...
        # --- MODEL 5: Comparison: ALL MODELS ---
        elif model_type == "Comparison: All Models":
    ...
    # --- PAGE 6: CONCLUSIONS ---
        ...
        * **Meaning:** Deterioration in the job market is a leading indicator. It signals a stock market downturn *before* it happens, validating the use of macro data for forecasting.
        ...
        ### 2. Model Comparison: Linear vs. Non-Linear
    ...
    # --- 7. REAL-WORLD ANALYSIS ---
    elif options == "7. Real-World Analysis":
        st.header("7. Real-World S&P 500 & Recession Analysis")
        st.write("This section analyzes real-world economic data, comparing the S&P 500 against major economic indicators and recession periods.")
        st.write("We have used the US Recession dataset which includes various economic indicators along with recession markers.")
        ...
        try:
            ...
            # --- DATE MAPPING LOGIC ---
            # Based on recession alignment (Index 312 = Feb 2020, Index 166 = Dec 2007)
            # We generate a date range starting from Feb 1994.
            if 'Date' not in df_real.columns:
                df_real['Date'] = pd.date_range(start='1994-02-01', periods=len(df_real), freq='MS')
            
            # 1. Label Mapping for Proper Labeling
            column_mapping = {
                ...
                "Rate": "Interest Rate",
                "Housing_Index": "Housing Market Index",
                "BBK_Index": "BBK Recession Probability Index",
                "Recession": "Recession Binary Indicator",
                "3 Mo": "3-Month Treasury Yield",
                "10 Yr": "10-Year Treasury Yield"
            }
            ...
            if selected_indicators:
                ...
                # Plot each selected variable using the 'Date' column for X-axis
                ...
                # Use the 'Date' column for the fill_between x-axis
                ...
                st.info("""
                **Timeline Analysis:**
                * The x-axis has been mapped to dates starting from **Feb 1994** based on the recession markers.
                * **2008 Crisis:** Visible around 2008 (Index ~166).
                * **2020 Recession:** Visible at the end of the timeline (Index ~312).
                """)
    ...
    # --- 8. REAL-WORLD ML MODELS ---
    elif options == "8. Real-World ML Models":
        st.header("8. Real-World Feature Engineering & ML")
        st.markdown("""
        Build your own Machine Learning models using the US Recession dataset.
        1. **Engineer Features:** Create lags and rolling averages.
        2. **Train Model:** Predict Recessions (Classification) or S&P 500 Price (Regression).
        """)
        ...
        try:
            ...
            with st.expander("Feature Engineering Options", expanded=True):
                st.write("Create new features to help the model learn patterns.")
                
                # Column Selection for FE
                ...
                # 1. Lag Features
                st.markdown("**1. Lag Generation** (Use past values to predict future)")
                lag_cols = st.multiselect("Select columns to create lags for:", numeric_cols, default=['Price_x', 'GDP'])
                n_lags = st.slider("Number of Months to Look Back (Lags):", 1, 12, 3)
                
                # 2. Rolling Statistics
                st.markdown("**2. Rolling Statistics** (Trend indicators)")
                roll_cols = st.multiselect("Select columns for Rolling Mean:", numeric_cols, default=['Price_x'])
                window_size = st.slider("Rolling Window Size (Months):", 2, 12, 6)
                ...
                if st.checkbox("Apply Feature Engineering"):
                    ...
                    for col in roll_cols:
                        df_ml[f'{col}_roll_mean_{window_size}'] = df_ml[col].rolling(window=window_size).mean()
                    
                    # Drop NaN values created by lags
                    df_ml.dropna(inplace=True)
                    st.success(f"Features created! Data shape after cleaning: {df_ml.shape}")
                    st.dataframe(df_ml.head())

            # --- SECTION 2: MODEL TRAINING ---
            st.subheader("Step 2: Model Training & Evaluation")
            
            model_type = "Classification" if target_col == 'Recession' else "Regression"
            
            if st.button(f"Train {model_type} Model"):
                # Prepare Data
                # Drop non-feature columns
                drop_cols = ['Date', 'Unnamed: 0'] 
                # Also drop the target column from X
                X = df_ml.drop(columns=drop_cols + [target_col])
                y = df_ml[target_col]
                
                # Time-based Split (Maintain order for time series)
                ...
                if model_type == "Classification":
                    ...
                    # Plot Results
                    fig, ax = plt.subplots(figsize=(10, 4))
                    ax.plot(y_test.index, y_test, label='Actual Recession', color='black')
                    ax.plot(y_test.index, preds, label='Predicted', color='red', linestyle='--')
                    ax.set_title("Recession Prediction (Test Set)")
                    ax.legend()
                    st.pyplot(fig)
                    ...
                    ax.plot(y_test.index, preds, label='Predicted Price', color='orange', linestyle='--')
                    ax.set_title("Price Forecasting (Test Set)")
                    ax.legend()
                    st.pyplot(fig)

                # Feature Importance (Applies to both)
                st.subheader("Feature Importance")
                importances = clf.feature_importances_ if model_type == "Classification" else reg.feature_importances_
                feat_df = pd.DataFrame({'Feature': X.columns, 'Importance': importances})
                feat_df = feat_df.sort_values(by='Importance', ascending=False).head(10)
                
                fig_imp, ax_imp = plt.subplots(figsize=(10, 5))
                sns.barplot(x='Importance', y='Feature', data=feat_df, ax=ax_imp, palette='viridis')
                ax_imp.set_title("Top 10 Most Predictive Features")
                st.pyplot(fig_imp)
    ...
    # --- 9. REAL-WORLD FORECASTING (ARIMA/VAR/LSTM) ---
    elif options == "9. Real-World Forecasting (ARIMA/VAR/LSTM)":
        st.header("9. Real-World Time Series Forecasting")
        st.write("Apply advanced time-series models to the real-world S&P 500 and Economic data.")

        # Load Data & Map Dates
        try:
            df_real = pd.read_csv("US_Recession.csv")
            # Date Mapping: Start Feb 1994 based on recession alignment
            if 'Date' not in df_real.columns:
                df_real['Date'] = pd.date_range(start='1994-02-01', periods=len(df_real), freq='MS')
            ...
            # --- MODEL 2: VAR ---
                ...
                selected_cols = st.multiselect("Select Variables for VAR System:", potential_cols, default=['Price_x', 'GDP', 'Rate'])
                ...
                if st.button("Train Real-World VAR"):
                    if len(selected_cols) < 2:
                        ...
                        try:
                            ...
                            st.info(f"Optimal Lag Selected: {selected_lag}")
                            model_fit = model.fit(selected_lag)
                            ...
                            # Metrics
                            rmse = np.sqrt(mean_squared_error(test_df[target_col], df_forecast[target_col + '_pred']))
                            mae = mean_absolute_error(test_df[target_col], df_forecast[target_col + '_pred'])
                            ...
                            ax.set_title("VAR Forecast: S&P 500 (Multivariate)")
                            ax.set_xlabel("Date")
                            ax.set_ylabel("Normalized Value")
                            ax.legend()
                            st.pyplot(fig)
            ...
            # --- MODEL 3: LSTM ---
                ...
                if st.button("Train Real-World LSTM"):
                    with st.spinner("Training LSTM..."):
                        ...
                        # Metrics
                        rmse = np.sqrt(mean_squared_error(actual_test, test_predict))
                        ...
                        # The y-array starts at index seq_length.
                        # The test set starts at train_split inside that y-array.
                        # So test data corresponds to original index: seq_length + train_split
            ...
            # --- MODEL 4: COMPARISON  ---
                ...
                st.subheader("Model Comparison: ARIMA vs VAR vs LSTM")
                st.markdown("Training all models on the same split to compare performance.")
                
                if st.button("Run Comparison"):
                    with st.spinner("Training ARIMA, VAR, and LSTM... this may take a moment."):
                        
                        # Common Data Setup
                        train_size = int(len(df_real) * train_size_pct)
                        train_df = df_real.iloc[:train_size]
                        test_df = df_real.iloc[train_size:]
                        ...
                        # 1. ARIMA Prediction
                        try:
                            arima_train = train_df[target_col]
                            arima_model = ARIMA(arima_train, order=(5,1,5)).fit()
                            ...
                            metrics_list.append(['ARIMA', rmse_a, mae_a])
                        except:
                            results['ARIMA'] = np.nan

                        # 2. VAR Prediction
                        try:
                            # Using default cols for comparison
                            var_cols = ['Price_x', 'GDP', 'Rate'] 
                            var_train = train_df[var_cols].dropna()
                            model_var = VAR(var_train)
                            lag = 5 # Fixed lag for speed/stability
                            fit_var = model_var.fit(lag)
                            
                            # Forecast
                            var_input = var_train.values[-lag:]
                            var_pred = fit_var.forecast(y=var_input, steps=len(test_df))
                            results['VAR'] = var_pred[:, 0] # Index 0 is Price_x
                            
                            rmse_v = np.sqrt(mean_squared_error(results['Actual'], results['VAR']))
                            mae_v = mean_absolute_error(results['Actual'], results['VAR'])
                            metrics_list.append(['VAR', rmse_v, mae_v])
                        except:
                            results['VAR'] = np.nan

                        # 3. LSTM Prediction
                        try:
                            # We need to map LSTM window logic to exact test dates
                            seq_length = 12
                            scaler = MinMaxScaler()
                            # Scale full data
                            full_series_scaled = scaler.fit_transform(df_real[target_col].values.reshape(-1, 1))
                            
                            # To predict the first point of TEST, we need the last `seq_length` points of TRAIN
                            # We construct inputs specifically for the test period
                            test_inputs = []
                            # We need to look back from the start of test data
                            # Full data index: train_size is start of test
                            for i in range(len(test_df)):
                                # Window is [train_size + i - seq_length : train_size + i]
                                start_idx = train_size + i - seq_length
                                window = full_series_scaled[start_idx : train_size + i]
                                test_inputs.append(window)
                            
                            test_inputs = np.array(test_inputs)
                            test_inputs = Variable(torch.Tensor(test_inputs))
                            
                            # Quick Train (simplified for battle speed)
                            # Train on sliding windows from training set only
                            def get_train_data(data, seq):
                                x, y = [], []
                            ...
                            train_scaled = full_series_scaled[:train_size]
                            x_train, y_train = get_train_data(train_scaled, seq_length)
                            ...
                            # Train loop
                            tx = Variable(torch.Tensor(x_train))
                            ty = Variable(torch.Tensor(y_train))
                            for _ in range(100):
                                out = lstm(tx)
                                opt.zero_grad()
                                loss = crit(out, ty)
                            ...
                            # Predict
                            lstm.eval()
                            lstm_pred_scaled = lstm(test_inputs).data.numpy()
                            results['LSTM'] = scaler.inverse_transform(lstm_pred_scaled).flatten()
                            
                            rmse_l = np.sqrt(mean_squared_error(results['Actual'], results['LSTM']))
                            mae_l = mean_absolute_error(results['Actual'], results['LSTM'])
                            metrics_list.append(['LSTM', rmse_l, mae_l])
                        except Exception as e:
                             st.write(f"LSTM Error: {e}")
                             results['LSTM'] = np.nan
                        ...
                        # 1. Metrics Table
                        metrics_df = pd.DataFrame(metrics_list, columns=['Model', 'RMSE (Lower is Better)', 'MAE'])
                        metrics_df.set_index('Model', inplace=True)
                        st.table(metrics_df.style.highlight_min(axis=0, color='lightgreen'))
                        
                        # 2. Comparison Plot
                        fig, ax = plt.subplots(figsize=(14, 7))
                        ax.plot(test_dates, results['Actual'], label='Actual Price', color='black', linewidth=2)
                        
                        if 'ARIMA' in results:
                            ax.plot(test_dates, results['ARIMA'], label='ARIMA', linestyle='--')
                        if 'VAR' in results:
                            ax.plot(test_dates, results['VAR'], label='VAR', linestyle='-.')
                        if 'LSTM' in results:
                            ax.plot(test_dates, results['LSTM'], label='LSTM', linestyle=':')
                            
                        ax.set_title("Model Comparison: Real-World S&P 500 Forecast")
                        ax.set_xlabel("Date")
                        ax.set_ylabel("Normalized Value")
                        ax.legend()
                        ax.grid(True, alpha=0.3)
